...
// import { IPage } from "../../app/api/pages";
// styles
...
IPrimaryNav) {
  console.log(pages);
  const [currentElementIndex, setCurrentelementIndex] = useState<null | number>(
    null
  );
  const [createPageIsActive, setCreatePageIsActive] = useState(false);

  // copy of page array for modifications
  const [pagesCopy, setPagesCopy] = useState<IPage[]>(
  ...
  useEffect(() => {
    ...
    const pagesForMenu = pagesCopy.map((page) => {
      ...
      const links = page.links
        ? (page.links.map((link) => ({
            target:
              pages.find((e) => e.id === link.target)?.name || "not found",
            value: link.value,
          })) as IPagesForMenuLink[])
        : [];

      const result = links.length > 0 ? { ...page, links } : page;
      return result;
    });
    setModifiedPages({
      ...
      <div className={styles.navButtonsContainer}>
        ...
        <div className={styles.navButtonsCollapseContainer}>
          <Collapse isOpen={createPageIsActive}>
            <NewPageForm
              pagesForSelect={modifiedPages.pagesForSelect}
      ...
      <ul className={styles.navList}>
        {modifiedPages.pagesForMenu.map((page, index) => (
          <Element
            ...
            active={currentElementIndex === index}
          />
        ))}
      </ul>
    </nav>
