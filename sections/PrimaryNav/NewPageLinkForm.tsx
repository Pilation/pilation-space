...
import { TRelationship } from '../../pages/api/organizations/[uuid]/pages'
import styles from '../../styles/sections/PrimaryNav/NewPageLinkForm.module.css'
...
const pagesRelationshipTypes = ['parent', 'child', 'interactsWith', 'encapsulates']

export default function NewPageLinkForm({ pagesList, handleAddLink }: INewPageLinkForm) {
  const [pageTo, setPageTo] = useState<string>('relatesto')
  const [relatesAs, setRelatesAs] = useState<TRelationship | 'relationships'>('relationships')
  ...
  const handlePageTo = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    !optionsAreSelected.pageTo && setOptionsAreSelected({ ...optionsAreSelected, pageTo: true })
  }
  const handleRelatesAs = (event: React.ChangeEvent<HTMLSelectElement>) => {
    setRelatesAs(event.target.value as TRelationship)
    !optionsAreSelected.relatesAs && setOptionsAreSelected({ ...optionsAreSelected, relatesAs: true })
  }
  ...
  const optionsAreSelectedChecker = Object.values(optionsAreSelected).every((e) => e === true)
  return (
    <div className={styles.newPageLinkForm}>
      <select className={styles.newPageLinkValueSelect} ref={selectPage} value={pageTo} onChange={handlePageTo}>
      ...
      {!optionsAreSelectedChecker ? (
        <button type='button' className={styles.newPageSubmitButton + ' button-disabled'} onClick={onFocusSelect}>
          Please fill links data
        </button>
      ) : (
        <button type='button' className={styles.newPageSubmitButton} onClick={handleClick}>
          Add link
        </button>
      )}
    </div>
